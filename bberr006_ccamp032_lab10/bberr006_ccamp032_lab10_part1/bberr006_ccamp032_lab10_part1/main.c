...
* Connect LEDs to PB0, PB1, PB2, and PB3. In one state machine (Three LEDs), output to a shared variable (threeLEDs) 
* the following behavior: set only bit 0 to 1, then only bit 1, then only bit 2 in sequence for 1 second each. 
* In a second state machine (Blinking LED), output to a shared variable (bilnkingLED) the following behavior: 
* set bit 3 to 1 for 1 second, then 0 for 1 second. In a third state machine (Combine LEDs), combine both shared variables 
* and output to the PORTB. Note: only one SM is writing to outputs. Do this for the rest of the quarter.
*
